...
/**
 * {@link HttpUtils} 基于{@link OkHttpClient}封装的http请求工具
...
public class HttpUtils {
	...
	public static Resp doGetWithHeader(String url,
	...
	public static Resp doGet(String url, Map<String, String> params) {
		...
		if (MapUtils.isNotEmpty(params)) {
			for (Map.Entry<String, String> entry : params.entrySet()) {
				builder.append(PARAM_SEPARATE).append(entry.getKey())
						.append(KV_SEPARATE).append(entry.getValue());
			}
		}
	...
	public static Resp executeRequest(Request request, int retryTime) {
		try {
			...
			return Resp.builder().code(response.code())
					...
					.message("Invoke failed, status code is not 200").build();
		} catch (Exception e) {
			if (retryTime-- > 0) {
				try {
					TimeUnit.MILLISECONDS.sleep(1000);
			...
			return Resp.builder().code(500)
					.message("Invoke occurred exception, request="
	...
	private static Result body2Result(ResponseBody body) throws IOException {
		return body == null ? Result.builder().code(-1).build()
				: new GsonBuilder().create().fromJson(body.string(),
						Result.class);
	}
